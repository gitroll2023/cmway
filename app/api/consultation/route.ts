  ...
  try {
    ...
    if (error) {
      return NextResponse.json(
        { 
          success: false, 
          error: typeof error === 'object' && error !== null && 'message' in error ? error.message : 'Failed to fetch consultations' 
        },
        { status: 500 }
      )
    }
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (error) {
      return NextResponse.json(
        { success: false, error: typeof error === 'object' && error !== null && 'message' in error ? error.message : 'Failed to update consultation' },
        { status: 500 }
      )
    }
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (error) {
      return NextResponse.json(
        { success: false, error: typeof error === 'object' && error !== null && 'message' in error ? error.message : 'Failed to delete consultation' },
        { status: 500 }
      )
    }
